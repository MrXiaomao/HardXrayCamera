...
bool HDADataUpload::connect()
{
    ...
    qint32 port_HDZSrv = settings->getValueByPath("network/port_hdaSrv", 9090).toInt();
...
bool HDADataUpload::startUploadSpectrumData(quint32 shot/*炮号*/,
                                            const std::string& shotTime/*打靶时刻*/,
                                            quint8 channel/*通道号*/,
                                            const std::vector<double>& time/*时间*/,
                                            const std::vector<double>& values/*能谱*/)
{
    // 写入数据
    {
        // 设置写入键
